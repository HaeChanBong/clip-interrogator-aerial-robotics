...
class Config:
    ...
    chunk_size: int = 64    # batch size for CLIP, use smaller for lower VRAM
    data_path: str = os.path.join(os.path.dirname(__file__), 'data')
    device: str = ("mps" if torch.backends.mps.is_available() else "cuda" if torch.cuda.is_available() else "cpu")
    env_intermediate_count: int = 64
...
class Interrogator():
    ...
    def interrogate_classic(self, image: Image, max_envs: int=3, caption: Optional[str]=None) -> str:
        ...
        if caption.startswith(res):
            prompt = f"A {positive} {frame} of {{}} in {env}, {caption}."
        else:
            prompt = f"A {res} {positive} {frame} of {{}} in {env}, {caption}."
